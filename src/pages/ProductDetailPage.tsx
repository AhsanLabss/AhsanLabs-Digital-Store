...
export const ProductDetailPage = ({ productId, onBack }: ProductDetailPageProps) => {
  ...
  const applyCoupon = (coupon: Coupon) => {
    ...
    if (coupon.discount === 100) {
      ...
      setDownloadLink('');
      setDiscountMessage(`Success! ${coupon.discount}% OFF applied with Coupon Code: "${coupon.code}" . Final price: $ ${newFinalPrice.toFixed(2)}`);
    }
  ...
  const handleBuyNow = () => {
    ...
    if (appliedCoupon && appliedCoupon.discount < 100) {
      ...
      message = `Hi I am interested to buy "${product.whatsappMessage}", (Price: $ ${product.price.toFixed(2)})`;
    }
